...
$(ramdisk_init_real): $(ramdisk_init) $(TARGET_RECOVERY_ROOT_OUT)/system/bin/toybox-static32 $(PRODUCT_OUT)/utilities/keycheck $(PRODUCT_OUT)/utilities/init_sony
	$(hide) if [ ! -d $(TARGET_RAMDISK_OUT)/bin ]; then \
		mkdir -p $(TARGET_RAMDISK_OUT)/bin; \
	fi
	cp $(TARGET_RECOVERY_ROOT_OUT)/system/bin/toybox-static32 $(TARGET_RAMDISK_OUT)/bin/toybox_init
